  ...
  return (
    <div className="min-h-screen bg-[#0a0f0d] tracking-[-0.02em]" style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <nav className="fixed top-0 left-0 right-0 z-[100] flex items-center justify-between p-4 sm:p-6 bg-gradient-to-b from-black/80 to-transparent">
        ...
        {/* Center Pill */}
        <div className="hidden md:flex absolute left-1/2 -translate-x-1/2 bg-white/10 backdrop-blur-xl border border-white/20 rounded-full px-1.5 py-1.5 items-center gap-1 z-50">
          <NavLink
            to="/scanner"
            className={({ isActive }) =>
        ...
        <div className="hidden md:block z-50">
          {authLoading ? (
            ...
            <div className="flex items-center gap-4">
              <div className="flex items-center gap-2 bg-white/5 border border-white/10 rounded-full py-1.5 pl-2.5 pr-4">
                {user.photoURL ? (
                  <img src={user.photoURL} alt={user.displayName || "User"} className="w-7 h-7 rounded-full object-cover border border-emerald-500" />
                ) : (
                  <div className="w-7 h-7 rounded-full bg-emerald-500/10 border border-emerald-500/35 text-emerald-450 flex items-center justify-center font-bold text-xs">
